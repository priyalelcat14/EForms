sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  "use strict";
  var value_obj;
  return BaseController.extend("spefar.app.controller.Dialog8", {
 ...
 _onButtonPress1: function() {
      ...
      var rec = this.getView().byId("searchcofa_inp").getValue();
      var relPath = "eFormLobs?$filter=SLOB_DESCRIPTION eq '"+rec+"'";
      var that = this;
      oModelData.read(relPath, null, [], false, function(oData, response) {
          that.getView().byId("cofa_lob_table").destroyItems();
          var counter = response.data.results.length;
          var i = 0;
          var otable = that.getView().byId("cofa_lob_table");
          for (i = 0; i < counter; i++) {
            var data = new sap.m.ColumnListItem({
              cells: [
                new sap.m.Text({
                  text: response.data.results[i].LOB
                }), new sap.m.Text({
    ...
    _onButtonPress3: function() {
      ...
      var selected_items = this.getView().byId("cofa_lob_table").getSelectedItems();
    ...
    onInit: function() {
      ...
      oModelData.read(relPath, null, [], false, function(oData, response) {
          ...
          var table = that.getView().byId("cofa_lob_table");
          for (i = 0; i < counter; i++) {
            var data = new sap.m.ColumnListItem({
